...
class ConfigManager(Singleton):
    ...
    def __init__(self, config_dir: str = "config", config_file: str = "config.yaml"):
        ...
        def _get_llm_components(self):
            """LLMコンポーネントを遅延インポートで取得"""
        try:
            from ..llm import LLMConfig, LLMProvider, get_available_providers, validate_llm_config
            return LLMConfig, LLMProvider, get_available_providers, validate_llm_config
        except ImportError as e:
            logger.warning(f"LLMコンポーネント読み込み失敗: {e}")
            return None, None, None, None
    
    def _load_llm_config(self, config_data: Dict[str, Any]) -> Dict[str, Any]:
        """LLM設定を読み込み（遅延インポート使用）"""
        try:
            LLMConfig, LLMProvider, get_available_providers, validate_llm_config = self._get_llm_components()
            
            if not all([LLMConfig, LLMProvider, get_available_providers, validate_llm_config]):
                logger.warning("LLMコンポーネントが利用できません")
            ...
            for provider_name, provider_config in providers.items():
                if provider_name in available_providers:
                    # LLMConfigオブジェクトを作成して検証
                    try:
                        config_obj = LLMConfig(**provider_config)
                        if validate_llm_config(config_obj):
                            logger.debug(f"プロバイダー設定を検証しました: {provider_name}")
                    except Exception as e:
                        logger.warning(f"プロバイダー設定検証エラー {provider_name}: {e}")
